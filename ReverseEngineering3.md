...
By the way, I "listened" to sdram.app and cipher result to figure out how much decryption successfully worked. You can find those in [misc](https://github.com/0x0-nyan/JL_AC46/blob/main/misc/) directory too.<br>

## ENC Cipher
Some contents in firmware image generated by isd_download are scrambled by ENC cipher with a key of 0xFFFF to hide those.<br>
The flash header, file list and user.app are scrambled in 32 bytes block, and uboot.boot is scrambled in 2 parts (one 16 bytes of BANKCB header and one BANKCB blob).<br>
